...
import { LogOut, Zap, Trophy, Smile, MessageSquare, User, LayoutDashboard, Megaphone, HandHelping, Shield } from "lucide-react";
...
const linksBase = [
  ...
  { to: "/shoutouts", label: "SHOUTOUTS", icon: Megaphone, color: "bg-brutal-yellow" },
...
export const Navbar = () => {
  ...
  if (user?.role === "admin") {
    links.push({ to: "/admin", label: "ADMIN", icon: Shield, color: "bg-black text-brutal-yellow" });
  }
  return (
    <header
      data-testid="navbar"
        ...
        <nav className="flex gap-1.5 flex-wrap" data-testid="nav-links">
          {links.map((l) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-1 font-black uppercase text-[11px] md:text-xs tracking-wider px-2.5 py-1.5 border-[3px] border-black rounded-[2px] transition-all ${
